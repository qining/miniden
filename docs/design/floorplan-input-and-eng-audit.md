...
日期：2026-09-17 · 状态：研究/提案（命名已定：MiniDen；未实施）
...
6. **命名决定：`planner` → `MiniDen`**（2026-09 定名，详见 §0.1）：产品身份是「自家房子的迷你模型，看家具放不放得下」，娃娃屋视角是标志性画面。
7. **产品调性（ADR-0006，2026-09-17）**：给 homeowner 做简单规划；「快捷」= time-to-insight（用户不建模、概念以分钟计；楼梯台阶突出量那类施工图域不做）；扩展代码优先（skill 是交付物非依赖，运行时零 AI）；范围内精度拉满。本文 §2–§4 按此重新校准。

---

## 0.1 命名决定：planner → MiniDen

实施启动前决定（2026-09-17）。

**含义**。den = 舒适私人小天地（a cozy nook），中文叫「迷你窝」。产品本质不是精确 CAD，而是自家房子的一个迷你模型——看家具放不放得下、布局长什么样；娃娃屋视角（掀掉屋顶往里看，产品的标志性画面）直接成为「MiniDen 视角」。

**命名过程**（每个候选都实测过同名撞车与域名占用）：

| 候选 | 含义 | 排除原因 |
|---|---|---|
| Rumplan | 瑞典语 room plan | 与俄罗斯户型设计服务「РумПлан」直接撞车（同品类） |
| Homeline / Floorform / Dwello | home+line / floor+form / dwell | 分别有活跃产品：CCTV 应用、英国地板承包商、Interac 支付 API |
| Planform / Trueplan / Roomline / Bostad | plan→form 等 | .com 全部注册于 1999–2005（早被占用） |
| **miniwo**（用户原案「迷你窝」pinyin） | 迷你窝 | 含义采纳，拼写已占：miniwo.app 是儿童英语词汇 App（App Store 在架），.com/.ai 于 2026 年接连被注册（有人在圈） |
| **MiniDen**（✅ 定名） | den = 窝 | 未发现同名活跃产品；miniden.com 为停放待售（可收购），**miniden.ai 未注册**（可注册） |

**执行步骤**（实施启动时一起做，或随时独立执行）：

1. GitHub 把 `qining/planner` → `qining/miniden`（旧 URL 自动重定向，现有链接不断）
2. `git remote set-url origin git@github.com:qining/miniden.git`
3. **本地目录名与文件名暂不动**：`/Users/dako/planner`、`planner.html`、AGENTS.md/测试台/check 脚本里的大量 `file://` 路径都与 repo 名无关，全改的成本（三个测试台 + check-item.py + AGENTS.md）现阶段不划算；等 P0-1 的 esbuild 架构落地时再一并定产物名
4. 页面顶部的产品展示名、README/docs 里的称谓，在第一次动 UI 时一并改为 MiniDen

**边界**：命名即定位。「不是特别精确、主要看东西放不放得下」正是从娃娃屋/迷你系而不是精度系取名原因。
