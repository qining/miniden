...
| # | 工作项 | 状态 | 备注 |
|---|---|---|---|
| S1 | **Project schema v1**：户型迁移为文档实例；`eff*()` 改读文档；id 寻址替换 USERGEO 索引层；旧数据一次性迁移 | **Phase 1 完**（`src/schema/` 纯函数 + 等价性 oracle 31 单测，1bb1637）；**Phase 2a 完**（eff* 文档驱动 + 双写持久化，本次提交）；2b（USERGEO 索引层退役）待做 | ADR-0005；**第一优先** |
