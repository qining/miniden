...
| 床/柜侧栏与前后板轴向写反 | 床长轴 = z(d)、宽 = x(w)：侧栏沿 z、床头/脚板跨 x；`C.box(w,h,d)` 容易把 len 写进第一参 | brief 先写死「长轴=z」；侧栏永远 `C.box(Th, h, len_z, …, ±(W/2−Th/2), …, z_c)`，床头/脚板跨 x 放 z=±D/2（UTÅKER、SAPIENS 连犯两次） |
...
| 2D 图例外框变椭圆 | `el('rect').rx` 是原始像素（R() 只乘 S 到 x/y/w/h），rx:5*S 把矮矩形撑成椭圆 | rx 用 2~4 常数 |
| 建出来的东西不像商品名称说的那个品类 | 没做名称交叉验证 | 验证第四视角：读商品名（含功能词），逐项指认模型里每个宣称功能（5-in-1→5 件都认得出），认不出=不像 |
| “N-in-1” 把名字暗示、实拍里没有的独立件建了出来（篮球架杆） | 名字是营销枚举，照片才是权威：5-in-1 的“篮球”其实是巴士侧大环兼投篮目标，87cm 高来自青 A 架顶，并没有独立 hoop 杆 | 对模型每个件反向核对产品照片：名字暗示的件若照片里没有，多是既有件的双用途/营销话术——先确认该功能实际由哪个件承担，高度 spec 归因到实际最高件，别为凑名字/凑高度凭空建杆 |
