...
**关 4 —— 和实拍图逐项比对（至少两个角度，硬条件）**（对着 `work/ref/` 里的图看）

`check-item.py <条目id>` 出两张图：主 3/4 前视角（`check_<id>.png`）和侧视角
（`check_<id>_side.png`）。**两张都要看、都要对照过**——但**参考图的视角是商品页上
实际存在的角度**（常见 3/4 斜视角、细节特写），不是写死的正面/侧面：对每张渲染图，
从参考图里**挑视角最接近的那张**逐项比（一张 3/4 实拍可以同时核对主视角和侧视角）。
侧面轮廓（腿距、悬挑、背部结构、深度比例）只有在能看到侧面的角度才露馅，单角度最容易在这栽。
某张渲染图找不到视角足够接近的参考图（例如背部结构核对不了）时，先补抓变体图/细节特写再对照。
